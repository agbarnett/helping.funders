...
shinyUI(fluidPage(
  ...
  p("Researchers are often asked by funders to give their publication list, but funders often have different requirements (e.g., only papers in last five years) and researchers waste a lot of time formating papers. ",
    ...
    "If any funders are interested I can set up a batch process just ", tags$a(href='mailto:a.barnett@qut.edu.au', 'e-mail'), ' me. Also please e-mail if you have any suggestions for improvement.', sep=''),
  
  sidebarLayout(
    sidebarPanel(
      textInput(inputId = "orcid.id",
                label = "ORCID ID (16 digits with dashes)",
